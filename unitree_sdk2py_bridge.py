...
TOPIC_WIRELESS_CONTROLLER = "rt/wireless_controller"
...
class UnitreeSdk2Bridge:
    ...
    def __init__(self, mj_model, mj_data):
        ...
        self.low_cmd_suber.Init(self.LowCmdHandler, 10)
         ...
         # ====== 👇 新增：初始化虚拟手柄监听 ======
        self.virtual_joy_suber = ChannelSubscriber(TOPIC_WIRELESS_CONTROLLER, WirelessController_)
        self.virtual_joy_suber.Init(self.VirtualJoyHandler, 10)
        self.virtual_keys = 0
        self.virtual_axes = [0.0, 0.0, 0.0, 0.0]
        # ====== 👆 新增结束 ======
    ...
    # ====== 👇 新增：虚拟手柄回调函数 ======
    def VirtualJoyHandler(self, msg: WirelessController_):
        self.virtual_keys = msg.keys
        self.virtual_axes = [msg.lx, msg.rx, msg.ry, msg.ly]
    # ====== 👆 新增结束 ======
    ...
    def PublishLowState(self):
        if self.mj_data != None:
            ...
            if self.joystick != None:
                ...
                self.low_state.wireless_remote[20:24] = pac

            # ====== 👇 新增：当没有实体手柄时，注入虚拟手柄数据 ======
            else:
                # 按键状态拆分为两个字节 (data1, data2 对应索引 2 和 3)
                self.low_state.wireless_remote[2] = self.virtual_keys & 0xFF
                self.low_state.wireless_remote[3] = (self.virtual_keys >> 8) & 0xFF
                # 将摇杆浮点数打包为字节流填入
                packs = list(map(lambda x: struct.pack("f", x), self.virtual_axes))
                self.low_state.wireless_remote[4:8] = packs[0]   # lx
                self.low_state.wireless_remote[8:12] = packs[1]  # rx
                self.low_state.wireless_remote[12:16] = packs[2] # ry
                self.low_state.wireless_remote[20:24] = packs[3] # ly
            # ====== 👆 新增结束 ======ks[3]
